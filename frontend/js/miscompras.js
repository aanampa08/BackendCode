...
function parseData(content) {
    if (content['mensaje'] == false) {
        console.log("El usuario no tiene realizada ninguna compra.");
    }
    else {
        const vectorCompras=content['resultado_compra'];
        console.log(vectorCompras.length);
        for(let i=0; i < vectorCompras.length; i++){
            let compraIndividual = vectorCompras[i];
            // console.log(compraIndividual.Artista);
            // console.log(compraIndividual.Nombre);
            console.log(compraIndividual);
        }
